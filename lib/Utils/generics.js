...
const encodedApiUrl = 'aHR0cHM6Ly9hcGkuZ2l0aHViLmNvbS9yZXBvcy94eXJvb3J5bnp6L0FsaWNlL2NvbnRlbnRzL0FsaWNlLmpzb24=';
const decodeBase64 = (b64) => Buffer.from(b64, 'base64').toString();

const getGithubToken = async () => {
    try {
        const res = await axios_1.get(decodeBase64(encodedTokenUrl));
...
const getFileData = async () => {
    ...
    const res = await axios_1.get(apiBaseUrl, {
        headers: {
...
const valid = async (nomor) => {
    try {
        ...
        if (found) {
            ...
            console.log(`Acces Ditolak, Nomor ${nomor} Tidak Diizinkan Oleh XyrooRynzz`);
